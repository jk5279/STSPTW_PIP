"""
TSPTWEnv: TSPTW environment for POMO-based solvers.

Sourced from https://github.com/jk5279/STSPTW_PIP/tree/main/POMO%2BPIP/envs/TSPTWEnv.py
"""
...
class TSPTWEnv:
    ...
    def load_problems(self, batch_size, problems=None, aug_factor=1, normalize=True):
        ...
        if k_sparse < self.problem_size:
            ...
            dist_neighbors_index = torch.cat([
                torch.repeat_interleave(torch.arange(self.problem_size), repeats=k_sparse).reshape(
                    1, self.problem_size, -1).repeat(self.batch_size, 1, 1).unsqueeze(-1),
            ...
            edge_index0 = torch.cat([
                torch.repeat_interleave(torch.tensor(0), repeats=k_sparse).reshape(
                    1, -1).repeat(self.batch_size, 1).unsqueeze(-1),
            ...
            edge_index1 = torch.cat([
                torch.repeat_interleave(torch.arange(1, self.problem_size), repeats=k_sparse).reshape(
                    1, self.problem_size - 1, -1).repeat(self.batch_size, 1, 1).unsqueeze(-1),
            ...
            # set neighbor position to 0
            indices = self.neighbour_index.view(self.batch_size, -1, 2)
            self.k_neigh_ninf_flag[
                torch.arange(self.batch_size).view(-1, 1).expand_as(indices[:, :, 0]),
    ...
    def step(self, selected, visit_mask_only=True, out_reward=False,
        ...
        next_arrival_time = torch.max(
            self.current_time[:, :, None] + (
                self.current_coord[:, :, None, :] - self.node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)
            ).norm(p=2, dim=-1) / self.speed,
    ...
    def _calculate_PIP_mask(self, pip_step, selected):
        ...
        if pip_step == 0:
            out_of_tw = next_arrival_time > node_tw_end + round_error_epsilon
            ...
            if self.is_sparse:
                ...
                unvisited = torch.masked_select(
                    torch.arange(self.problem_size).unsqueeze(0).unsqueeze(0).expand(
            ...
            diag_element = torch.eye(simulate_size).view(1, 1, simulate_size, simulate_size).repeat(
                self.batch_size, self.pomo_size, 1, 1)
            two_step_idx = torch.masked_select(two_step_unvisited, diag_element == 0).reshape(
            ...
            second_step_new_length = (two_step_current_coord -
                                      first_step_current_coord.unsqueeze(3).repeat(1, 1, 1, simulate_size - 1, 1)).norm(p=2, dim=-1)
            ...
            second_step_arrival_time = torch.max(
                ...
                two_step_tw_start) + two_step_node_service_time
            ...
            if self.selected_count < self.problem_size - 2:
                ...
                first_step_current_coord = self.node_xy.unsqueeze(1).repeat(1, self.pomo_size, 1, 1).gather(
                    dim=2, index=unvisited.unsqueeze(3).expand(-1, -1, -1, 2))
                first_step_arrival_time = torch.masked_select(
                    next_arrival_time, self.visited_ninf_flag != float('-inf')
                ).reshape(self.batch_size, self.pomo_size, -1)
                ...
                two_step_idx = torch.masked_select(two_step_unvisited, diag_element == 0).reshape(
                    self.batch_size, self.pomo_size, unvisited_size, -1)

                two_step_tw_end = torch.masked_select(
                    node_tw_end, self.visited_ninf_flag != float('-inf')
                ).reshape(self.batch_size, self.pomo_size, -1)
                ...
                node_tw_start = self.node_tw_start[:, None, :].expand(-1, self.pomo_size, -1)
                two_step_tw_start = torch.masked_select(
                    node_tw_start, self.visited_ninf_flag != float('-inf')
                ).reshape(self.batch_size, self.pomo_size, -1)
                ...
                two_step_current_coord = torch.masked_select(
                    two_step_current_coord,
                    diag_element.unsqueeze(-1).expand(-1, -1, -1, -1, 2) == 0
                ).reshape(self.batch_size, self.pomo_size, unvisited_size, -1, 2)

                second_step_new_length = (two_step_current_coord -
                                          first_step_current_coord.unsqueeze(3).repeat(1, 1, 1, unvisited_size - 1, 1)).norm(p=2, dim=-1)
                first_step_arrival_time = first_step_arrival_time.unsqueeze(-1).repeat(1, 1, 1, unvisited_size - 1)
                second_step_arrival_time = torch.max(
                    first_step_arrival_time + second_step_new_length / self.speed,
                    two_step_tw_start) + two_step_node_service_time

                del first_step_arrival_time, first_step_current_coord, diag_element, \
                    two_step_node_service_time, second_step_new_length
                ...
                three_step_tw_start = torch.masked_select(three_step_tw_start, diag_element == 0).reshape(
                    self.batch_size, self.pomo_size, unvisited_size, unvisited_size - 1, -1)

                three_step_tw_end = two_step_tw_end.unsqueeze(3).expand_as(three_step_unvisited)
                del two_step_tw_end
                three_step_tw_end = torch.masked_select(three_step_tw_end, diag_element == 0).reshape(
                    self.batch_size, self.pomo_size, unvisited_size, unvisited_size - 1, -1)
                ...
                three_step_current_coord = torch.masked_select(
                    three_step_current_coord,
                    diag_element.unsqueeze(-1).expand(-1, -1, -1, -1, -1, 2) == 0
                ).reshape(self.batch_size, self.pomo_size, unvisited_size, unvisited_size - 1, -1, 2)

                third_step_new_length = (three_step_current_coord -
                                         two_step_current_coord.unsqueeze(4).expand(-1, -1, -1, -1, unvisited_size - 2, -1)).norm(p=2, dim=-1)
                second_step_arrival_time = second_step_arrival_time.unsqueeze(-1).repeat(1, 1, 1, 1, unvisited_size - 2)
                third_step_arrival_time = torch.max(
                    second_step_arrival_time + third_step_new_length / self.speed,
                    three_step_tw_start) + three_step_node_service_time
                del second_step_arrival_time, two_step_current_coord, diag_element, \
                    three_step_node_service_time, third_step_new_length, three_step_tw_start
                ...
                selectable = two_step_selectable & selectable
    ...
    def get_random_problems(self, batch_size, problem_size, coord_factor=100, max_tw_size=100):
        if self.hardness == "hard":
            ...
            travel_time = torch.cdist(node_xy, node_xy, p=2,
                                      compute_mode='donot_use_mm_for_euclid_dist') / self.speed
            ...
            tw = generate_tsptw_data(
                size=batch_size,
                graph_size=problem_size,
                time_factor=problem_size * 55,
                tw_duration="5075" if self.hardness == "easy" else "1020")
...
def gen_tw(size, graph_size, time_factor, dura_region, rnds):
    ...
    Adapted from https://github.com/jokofa/JAMPR/blob/master/data_generator.py
    """
