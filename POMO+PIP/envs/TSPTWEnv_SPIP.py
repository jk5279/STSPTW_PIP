...
class TSPTWEnv_SPIP:
    def __init__(self, **env_params):
        ...
        self.spip_sigma0 = env_params.get('spip_sigma0', 0.3)
        self.spip_epsilon = env_params.get('spip_epsilon', 0.05)
        # If True, transition uses realized travel = d + ξ (bounded noise). Default False = deterministic transition.
        self.spip_stochastic_transition = env_params.get('spip_stochastic_transition', False)
        self.spip_noise_bound = env_params.get('spip_noise_bound', 2.0)
        self.spip_noise_dist = env_params.get('spip_noise_dist', 'uniform')  # 'uniform' or 'clipped_gaussian'
    ...
    def _sample_bounded_noise(self, distance):
        ...
        d = distance.clamp(min=1e-8)
        scale = torch.sqrt(d)
        if self.spip_noise_dist == 'uniform':
            # Uniform on [-bound * sqrt(d), +bound * sqrt(d)]
            half_width = self.spip_noise_bound * scale
            xi = (torch.rand_like(distance) * 2.0 - 1.0) * half_width
        elif self.spip_noise_dist == 'clipped_gaussian':
            # Gaussian scale σ₀√d, clipped to ±bound*√d
            sigma_scale = self.spip_sigma0 * scale
            xi = torch.randn_like(distance) * sigma_scale
            bound_val = self.spip_noise_bound * scale
            xi = xi.clamp(min=-bound_val, max=bound_val)
        else:
            raise ValueError("spip_noise_dist must be 'uniform' or 'clipped_gaussian', got %s" % self.spip_noise_dist)
        return xi
    ...
    def load_problems(self, batch_size, problems=None, aug_factor=1, normalize=True):
        ...
        ).unsqueeze(0).repeat(self.batch_size, 1, 1) * (1e9)
        distances += diag_mask
        ...
        if k_sparse < self.problem_size:
            ...
            dist_neighbors_index = torch.cat([
                torch.repeat_interleave(
                    torch.arange(self.problem_size, device=topk_indices1.device),
                    repeats=k_sparse,
                ).reshape(1, self.problem_size, -1).repeat(self.batch_size, 1, 1).unsqueeze(-1),
            ...
            edge_index0 = torch.cat([
                torch.repeat_interleave(
                    torch.tensor(0, device=topk_indices2.device),
                    repeats=k_sparse,
            ...
            eye_matrix = torch.eye(self.problem_size - 1, device=overlap_matrix.device).unsqueeze(0).repeat(self.batch_size, 1, 1).bool()
            ...
            edge_index1 = torch.cat([
                torch.repeat_interleave(
                    torch.arange(1, self.problem_size, device=topk_indices3.device),
                    repeats=k_sparse,
                ).reshape(1, self.problem_size - 1, -1).repeat(self.batch_size, 1, 1).unsqueeze(-1),
            ...
            batch_arange = torch.arange(self.batch_size, device=indices.device).view(-1, 1).expand_as(indices[:, :, 0])
            self.k_neigh_ninf_flag[batch_arange, indices[:, :, 0], indices[:, :, 1]] = 0
            self.k_neigh_ninf_flag[batch_arange, indices[:, :, 1], indices[:, :, 0]] = 0
        else:
    ...
    def reset(self):
        ...
        # shape: (batch, pomo)
        self.cumulative_realized_travel_time = torch.zeros(size=(self.batch_size, self.pomo_size)).to(self.device)
        # shape: (batch, pomo) — sum of travel_time_for_step each step; used for distance reward when stochastic/delay
    ...
    def step(self, selected, visit_mask_only =True, out_reward = False, generate_PI_mask=False, use_predicted_PI_mask=False, pip_step=1):
        ...
        current_coord = self.node_xy[batch_arange, selected]
        # shape: (batch, pomo, 2)
        new_length = (current_coord - self.current_coord).norm(p=2, dim=-1)
        # shape: (batch, pomo)
        # S-PIP stochastic transition: realized_length = d + ξ (bounded noise when enabled)
        if self.spip_stochastic_transition and (self.spip_sigma0 > 0 or self.spip_noise_bound > 0):
            xi = self._sample_bounded_noise(new_length)
            d_ij = new_length.clamp(min=1e-8)
            realized_length = (d_ij + xi).clamp(min=1e-8)
            new_length = realized_length
        self.length = self.length + new_length
        self.current_coord = current_coord

        # Travel time for current_time update (no vrp_bench in this repo)
        travel_time_for_step = new_length / self.speed

        self.cumulative_realized_travel_time = self.cumulative_realized_travel_time + travel_time_for_step
        ...
        self.current_time = (
            torch.max(self.current_time + travel_time_for_step, self.node_tw_start[batch_arange, selected])
            + self.node_service_time[batch_arange, selected]
        )
        # shape: (batch, pomo)
        ...
        total_timeout = self.current_time - self.node_tw_end[batch_arange, selected]
        ...
        if done:
            # Distance component: realized travel time (matches stochastic/delay transition); deterministic case = total_dist/speed
            dist_component = self.cumulative_realized_travel_time
            if not out_reward:
                reward = -dist_component  # note the minus sign!
            else:
                # shape: (batch, pomo)
                dist_reward = -dist_component  # note the minus sign
    ...
    def _calculate_PIP_mask(self, pip_step, selected):
        # S-PIP: path-level buffer and incremental lookahead buffer (PIP_vs_SPIP.md)
        round_error_epsilon = 0.00001
        sigma0 = self.spip_sigma0
        probit = self.z_factor  # precomputed in __init__ to avoid scipy in step()

        # Step 1: Path distance to candidate j (all j)
        d_ij = (self.current_coord[:, :, None, :] - self.node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)).norm(p=2, dim=-1)  # (batch, pomo, problem)
        L_k = self.length  # (batch, pomo)
        L_j = (L_k.unsqueeze(-1) + d_ij).clamp(min=1e-8)  # (batch, pomo, problem)

        # Step 2: Path buffer and buffered first-step arrival
        delta_path = probit * sigma0 * torch.sqrt(L_j)
        arrival_raw = self.current_time[:, :, None] + d_ij / self.speed + delta_path
        ...
        t_hat_1_j = torch.max(arrival_raw, node_tw_start)
        out_of_tw_local = t_hat_1_j > node_tw_end + round_error_epsilon

        if pip_step == 0:
            if self.is_sparse:
                print("Warning! Performing zero-step S-PIP masking on k nearest neighbors is not supported! Consider all the unvisited nodes instead!")
            self.simulated_ninf_flag = torch.zeros((self.batch_size, self.pomo_size, self.problem_size), device=self.device)
            self.simulated_ninf_flag[out_of_tw_local] = float('-inf')
        elif pip_step == 1:
            ...
            if self.is_sparse:
                ...
                self.visited_and_notneigh_ninf_flag = (self.not_neigh_ninf_flag == 0) & (self.visited_ninf_flag == 0)
                unvisited_and_neigh_counts = self.visited_and_notneigh_ninf_flag.sum(dim=-1)
                max_count = unvisited_and_neigh_counts.max().item()
                _, unvisited_and_neigh = torch.topk(self.visited_and_notneigh_ninf_flag.int(), dim=-1, largest=True, k=max_count)
                unvisited = unvisited_and_neigh.sort(dim=-1)[0]
                del unvisited_and_neigh, unvisited_and_neigh_counts
            else:
                unvisited = torch.masked_select(torch.arange(self.problem_size, device=self.device).unsqueeze(0).unsqueeze(0).expand(self.batch_size, self.pomo_size, self.problem_size),
            ...
            diag_element = torch.eye(simulate_size, device=self.device).view(1, 1, simulate_size, simulate_size).repeat(self.batch_size, self.pomo_size, 1, 1)
            ...
            first_step_arrival_time = t_hat_1_j.gather(dim=-1, index=unvisited)
            L_j_first = L_j.gather(dim=-1, index=unvisited)
            ...
            node_tw_start_exp = self.node_tw_start[:, None, :].expand(-1, self.pomo_size, -1)
            two_step_tw_start = node_tw_start_exp.gather(dim=-1, index=unvisited)
            ...
            L_jm = L_j_first.unsqueeze(-1) + second_step_new_length
            L_jm = L_jm.clamp(min=1e-8)
            L_j_first_clamp = L_j_first.unsqueeze(-1).clamp(min=1e-8)
            delta_increment = probit * sigma0 * (torch.sqrt(L_jm) - torch.sqrt(L_j_first_clamp))
            first_step_arrival_time_exp = first_step_arrival_time.unsqueeze(-1).repeat(1, 1, 1, simulate_size - 1)
            second_step_arrival_time = torch.max(first_step_arrival_time_exp + second_step_new_length / self.speed + delta_increment, two_step_tw_start) + two_step_node_service_time

            infeasible_mark = (second_step_arrival_time > two_step_tw_end + round_error_epsilon)
            selectable = (infeasible_mark == False).all(dim=-1)

            self.simulated_ninf_flag = torch.full(
            ...
            # pip_step 2: deterministic (no S-PIP buffer) per plan
            if self.selected_count < self.problem_size - 2:
                next_arrival_time_det = torch.max(
                    self.current_time[:, :, None] + (self.current_coord[:, :, None, :] - self.node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)).norm(p=2, dim=-1) / self.speed,
                    self.node_tw_start[:, None, :].expand(-1, self.pomo_size, -1),
                )
                # unvisited nodes
                unvisited = torch.masked_select(
                    torch.arange(self.problem_size, device=self.device).unsqueeze(0).unsqueeze(0).expand(self.batch_size, self.pomo_size, self.problem_size),
                ...
                first_step_current_coord = self.node_xy.unsqueeze(1).repeat(1, self.pomo_size, 1, 1).gather(dim=2, index=unvisited.unsqueeze(3).expand(-1, -1, -1, 2))
                first_step_arrival_time = torch.masked_select(next_arrival_time_det, self.visited_ninf_flag != float('-inf')).reshape(self.batch_size, self.pomo_size, -1)
                ...
                delta_t = self.env_params.get("random_delta_t", 0.0) * torch.rand(
                    size=third_step_arrival_time.size(),
                    device=third_step_arrival_time.device,
                )
    ...
    def get_random_problems(self, batch_size, problem_size, coord_factor=100, max_tw_size=100):
        # Unused in this repo (source is never 'stsptw'); kept for compatibility. Requires stsptw_adapter if enabled.
        if self.env_params.get('source') == 'stsptw':
            from stsptw_adapter import generate_tsptw_from_stsptw
            num_workers = self.env_params.get('stsptw_num_workers', 1)
            return generate_tsptw_from_stsptw(
                batch_size, problem_size, self.hardness, num_workers=num_workers,
                verbose=self.env_params.get('verbose', False)
            )
