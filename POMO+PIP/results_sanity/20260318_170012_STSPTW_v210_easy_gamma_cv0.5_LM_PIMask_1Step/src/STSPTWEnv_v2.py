...
Design principles:
  ...
  - PIP mask uses Monte Carlo probability estimation against chance constraint alpha
...
class STSPTWEnv_v2:
    ...
    def __init__(self, **env_params):
        ...
        self.alpha = env_params.get('alpha', 0.95)    # chance constraint level
        self.n_mc_samples = env_params.get('n_mc_samples', 32)
    ...
    def _sample_travel_time_mc(self, distance, n_samples=None):
        """
        Sample multiple MC realizations for probability estimation.

        Args:
            distance: tensor of shape (...)
            n_samples: number of MC draws (default: self.n_mc_samples)
        Returns:
        ...
        if self.cv <= 0:
            return (distance / self.speed).unsqueeze(-1).expand(
                *distance.shape, n_samples)

        expanded = distance.unsqueeze(-1).expand(*distance.shape, n_samples)
        safe_dist = expanded.clamp(min=1e-8)
        ...
        travel = torch.where(expanded < 1e-8, torch.zeros_like(travel), travel)
        return travel / self.speed
    ...
    def _calculate_PIP_mask(self, pip_step, selected,
        ...
        """
        Extended PIP mask using Monte Carlo probability estimation.

        Mask node j if there exists k in U\\{j} such that:
            Pr[ max(a_k, tau_j + t_jk) + s_k > b_k ] > 1 - alpha
        where
            tau_j = max(a_j, current_time + t_tj) + s_j
        and t_tj, t_jk ~ F  are stochastic travel times.
        """
        round_error_epsilon = 0.00001
        ...
        if pip_step == 0:
            # Zero-step: pure reachability (deterministic, same as TSPTWEnv)
            if self.is_sparse:
            ...
            # One-step lookahead with MC stochastic feasibility
            self._pip_step1_mc(selected, next_arrival_time,
                               det_pairwise_dist, node_tw_end,
                               round_error_epsilon)
        else:
            raise NotImplementedError(
                f"pip_step={pip_step} not implemented for stochastic env")

    def _pip_step1_mc(self, selected, next_arrival_time,
                      det_pairwise_dist, node_tw_end, eps):
        """
        One-step-lookahead PIP mask via Monte Carlo.

        For each candidate first-step node j in the unvisited set U:
          For each second-step node k in U \\ {j}:
            Sample N travel times t_tj ~ F(d_tj) and t_jk ~ F(d_jk).
            Compute  tau_j  = max(a_j, current_time + t_tj) + service_j
            Compute  arr_k  = max(a_k, tau_j + t_jk) + service_k
            If Pr[arr_k > b_k] > 1 - alpha  for ANY k  →  mask j.
        """
        ...
        S = unvisited.size(-1)  # |U|

        if S <= 1:
            # Only one node left — no lookahead needed
            self.simulated_ninf_flag = torch.zeros(
                (self.batch_size, self.pomo_size, self.problem_size))
            return

        # ---- Gather node attributes for unvisited set ----
        node_tw_start_bp = self.node_tw_start[:, None, :].expand(
            -1, self.pomo_size, -1)
        node_svc_bp = self.node_service_time[:, None, :].expand(
            -1, self.pomo_size, -1)

        first_tw_start = node_tw_start_bp.gather(dim=-1, index=unvisited)
        first_svc = node_svc_bp.gather(dim=-1, index=unvisited)
        # (batch, pomo, S)

        # Coordinates of unvisited nodes
        first_coords = (
            self.node_xy.unsqueeze(1)
            .repeat(1, self.pomo_size, 1, 1)
            .gather(dim=2,
                    index=unvisited.unsqueeze(3).expand(-1, -1, -1, 2))
        )  # (batch, pomo, S, 2)

        # Distance current → j  (deterministic, for MC sampling base)
        dist_to_j = det_pairwise_dist.gather(dim=-1, index=unvisited)
        # (batch, pomo, S)

        # ---- Build second-step pairs (j, k) where k ∈ U \ {j} ----
        diag = (torch.eye(S).view(1, 1, S, S)
                .repeat(self.batch_size, self.pomo_size, 1, 1))

        # tw_end for second-step
        tw_end_u = node_tw_end.gather(dim=-1, index=unvisited)
        tw_end_u_rep = tw_end_u.unsqueeze(2).repeat(1, 1, S, 1)
        two_tw_end = torch.masked_select(
            tw_end_u_rep, diag == 0
        ).reshape(self.batch_size, self.pomo_size, S, -1)
        # (batch, pomo, S, S-1)

        # tw_start for second-step
        tw_start_u = first_tw_start.unsqueeze(2).repeat(1, 1, S, 1)
        two_tw_start = torch.masked_select(
            tw_start_u, diag == 0
        ).reshape(self.batch_size, self.pomo_size, S, -1)

        # service for second-step
        svc_u = first_svc.unsqueeze(2).repeat(1, 1, S, 1)
        two_svc = torch.masked_select(
            svc_u, diag == 0
        ).reshape(self.batch_size, self.pomo_size, S, -1)

        # Coordinates for second-step nodes
        coords_rep = first_coords.unsqueeze(2).repeat(1, 1, S, 1, 1)
        two_coords = torch.masked_select(
            coords_rep,
            diag.unsqueeze(-1).expand(-1, -1, -1, -1, 2) == 0
        ).reshape(self.batch_size, self.pomo_size, S, -1, 2)
        # (batch, pomo, S, S-1, 2)

        # Distance j → k  (deterministic base for MC sampling)
        dist_j_to_k = (
            two_coords
            - first_coords.unsqueeze(3).repeat(1, 1, 1, S - 1, 1)
        ).norm(p=2, dim=-1)
        # (batch, pomo, S, S-1)

        # ---- Monte Carlo sampling ----
        # t_tj samples: (batch, pomo, S, n_mc)
        mc_t_tj = self._sample_travel_time_mc(dist_to_j, n_mc)

        # t_jk samples: (batch, pomo, S, S-1, n_mc)
        mc_t_jk = self._sample_travel_time_mc(dist_j_to_k, n_mc)

        # tau_j = max(a_j, current_time + t_tj) + service_j
        cur_time = self.current_time[:, :, None, None].expand(
            -1, -1, S, n_mc)
        tw_start_j = first_tw_start[:, :, :, None].expand(-1, -1, -1, n_mc)
        svc_j = first_svc[:, :, :, None].expand(-1, -1, -1, n_mc)

        mc_tau_j = torch.max(cur_time + mc_t_tj, tw_start_j) + svc_j
        # (batch, pomo, S, n_mc)

        # arr_k = max(a_k, tau_j + t_jk) + service_k
        mc_tau_j_exp = mc_tau_j.unsqueeze(3).expand(-1, -1, -1, S - 1, -1)
        tw_start_k = two_tw_start.unsqueeze(-1).expand(-1, -1, -1, -1, n_mc)
        svc_k = two_svc.unsqueeze(-1).expand(-1, -1, -1, -1, n_mc)

        mc_arr_k = torch.max(mc_tau_j_exp + mc_t_jk, tw_start_k) + svc_k
        # (batch, pomo, S, S-1, n_mc)

        # Check: arr_k > b_k
        tw_end_k = two_tw_end.unsqueeze(-1).expand(-1, -1, -1, -1, n_mc)
        mc_infeasible = mc_arr_k > tw_end_k + eps
        # (batch, pomo, S, S-1, n_mc)

        # Pr[infeasible] per (j, k) pair
        prob_infeas = mc_infeasible.float().mean(dim=-1)
        # (batch, pomo, S, S-1)

        # Mask j if ANY k has violation probability > 1 - alpha
        any_k_risky = (prob_infeas > (1.0 - self.alpha)).any(dim=-1)
        # (batch, pomo, S)
        selectable = ~any_k_risky

        # ---- Write to simulated_ninf_flag ----
        self.simulated_ninf_flag = torch.full(
            (self.batch_size, self.pomo_size, self.problem_size),
            float('-inf'))
        sel_idx = selectable.nonzero(as_tuple=False)
        if sel_idx.numel() > 0:
            node_idx = unvisited[sel_idx[:, 0], sel_idx[:, 1], sel_idx[:, 2]]
            self.simulated_ninf_flag[
                sel_idx[:, 0], sel_idx[:, 1], node_idx] = 0.
