#!/bin/bash
# ============================================================
# STSPTW_v2 N=100 full training sweep (pre-decision noise only)
...
# Submit: sbatch scripts/train/sweep_v2_n100.sh
# ============================================================
#SBATCH --job-name=sv2_n100
#SBATCH --output=logs/slurm/sweep_v2_n100_%A_%a.out
#SBATCH --array=0-53%8
#SBATCH --nodes=1
...
echo "=== SLURM array job (v2 n100) started: A=${SLURM_ARRAY_JOB_ID}, a=${SLURM_ARRAY_TASK_ID} at $(date) ==="
...
echo "============================================"
echo "=== v2 n100  model=${MODEL}  pre-decision  ${NOISE}  cv=${CV}  ${HARDNESS} ==="
...
srun python train.py \
    ...
    --model_type "$MODEL" \
    --epochs 10000 \
    --train_episodes 10000 --train_batch_size 512 \
    --val_episodes 10000 --validation_batch_size 1000 \
    --validation_interval 500 --model_save_interval 50 \
    --simulation_stop_epoch 100 \
    --pip_update_epoch 20 \
    --log_dir "./results/sweep_v2_n100"
