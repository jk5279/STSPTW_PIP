#!/bin/bash
# Re-run only tasks 28-31 (same config as main array, no reveal_delay_before_action).
# Submit: sbatch --array=28-31 run_train_stochastic_n10_array_28_31.sh
#SBATCH --job-name=stsp_n10_28_31
#SBATCH --output=logs/slurm/stsp_n10_arr_28_31_%A_%a.out
#SBATCH --nodes=1
...
echo "=== SLURM array job (catch-up 28-31) started: A=${SLURM_ARRAY_JOB_ID}, a=${SLURM_ARRAY_TASK_ID} at $(date) ==="
...
# Same index mapping as run_train_stochastic_n10_array.sh (0..89)
...
echo "=== TASK $TASK_ID: mode=$MODE, hardness=$HARDNESS, delay_weight=$DW (no reveal) ==="
